...
def structural_registration(template,
                            ...
                            s_phases=['s_translation', 'rigid', 'affine'],
                            ):
    # if highres atlas registration
    if highresflag:
        s_phases = ['syn']
    elif presurgery:        # If presurgery, we want to keep the distances same
        s_phases = ['s_translation', 'rigid', "affine"]
        reference_template = template
    elif elastic:
        s_phases = ['s_translation', 'rigid', 'affine', 'syn']
    ...
    if moving_img_mask:
        s_registration.inputs.moving_image_masks = [path.abspath(path.expanduser(moving_img_mask))]
    s_registration.inputs.num_threads = num_threads

    s_warp = pe.Node(ants.ApplyTransforms(), name=name+"_warp")
    s_warp.inputs.reference_image = path.abspath(path.expanduser(reference_template))
    ...
    if presurgery:
        s_warp.inputs.out_postfix = "_RigidRegistered"
    else:
        s_warp.inputs.out_postfix = "_AffineRegistered"
...
def highres_warp_node(template, name="highres_warp", num_threads=4):
    highres_warp = pe.Node(ants.ApplyTransforms(), name=name)
