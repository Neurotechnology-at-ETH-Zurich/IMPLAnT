...
def get_bids_scan(data_selection,
	...
	if filtered_data.empty:
		...
		if nii_name[-7:] != '.nii.gz' and nii_name[-4:] != '.nii' and "-ind_" not in nii_name:
			# output_filename = '{}.nii.gz'.format(output_filename)
			# if transformation:
			# 	nii_name = nii_name + "-ind_" + transformation + "Registered"
			# else:
			nii_name = nii_name + "-ind_" + str(ind_type)
		...
		if "-ind_" not in metadata_filename:
