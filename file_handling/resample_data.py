...
class ResampleData:
    ...
    def resampling50um_trajectoryPlanning(file_name,new_spacing_mm=0.05):
        ...
        """
        img = sitk.ReadImage(file_name)
        old_spacing = img.GetSpacing() #x,y,z
        old_size = img.GetSize()
        # first resample in z
        new_spacing = [old_spacing[0],old_spacing[1],new_spacing_mm] #x,y,z
        ...
        old_spacing = resampled_z.GetSpacing()
        old_size = resampled_z.GetSize()
        new_spacing = [old_spacing[0],new_spacing_mm,old_spacing[2]] #x,y,z
        ...
        old_spacing = resampled_zy.GetSpacing()
        old_size = resampled_zy.GetSize()
        new_spacing = [new_spacing_mm, old_spacing[1],old_spacing[2]]
        ...
        filename_end = f"resampled{new_spacing_mm*1000:.10g}um.nii.gz"

        default_name = f"{file_name[:-7]}_{filename_end}"
        save_path = os.path.join(default_name)
